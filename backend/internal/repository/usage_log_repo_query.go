...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		CreatedAt:                 createdAt,
		TimingBreakdown:           usageTimingBreakdownFromNullJSON(timingBreakdown),
	}
	...
	if proxyName.Valid {
		log.ProxyName = &proxyName.String
	}
	if proxyHost.Valid {
		log.ProxyHost = &proxyHost.String
	}
	if proxyPort.Valid {
		value := int(proxyPort.Int64)
		log.ProxyPort = &value
	}
...
func nullUsageTimingBreakdownJSON(v *service.UsageTimingBreakdown) any {
	if v == nil {
		return nil
	}
	payload, err := json.Marshal(v)
...
func usageTimingBreakdownFromNullJSON(v sql.NullString) *service.UsageTimingBreakdown {
	if !v.Valid || strings.TrimSpace(v.String) == "" {
		return nil
	}
	var out service.UsageTimingBreakdown
	if err := json.Unmarshal([]byte(v.String), &out); err != nil {
		return nil
	}
	return &out
